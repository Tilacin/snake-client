...
public class Tail : MonoBehaviour
{
    ...
    [SerializeField] private float _detaliDistance = 1;
    ...
    private void Update()
    {
       ...
       while (distance > _detaliDistance)
        {
            Vector3 direction = (_head.position - _positionHistory[0]).normalized;

            _positionHistory.Insert(0, _positionHistory[0] + direction * _detaliDistance);
            _positionHistory.RemoveAt(_positionHistory.Count - 1);

            _rotationHistory.Insert(0, _head.rotation);
            _rotationHistory.RemoveAt(_rotationHistory.Count-1);

            distance -= _detaliDistance;
        }
        for (int i = 0; i < _details.Count; i++)
        {
            float percent = distance / _detaliDistance;
